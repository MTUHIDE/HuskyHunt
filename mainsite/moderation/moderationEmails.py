...
def sendRemoveRideEmail(ride, reason, suspensionDuration):
    ...
    message = (
            'Hello ' + user.first_name + ',\n\n' +
            'Your ride to ' + ride.destination_city + ' on ' + ride.date_leaving.strftime(
        "%m/%d/%Y") + ' has been removed by a moderator from our site.' +
...
def sendApproveRideEmail(ride):
    ...
    message = (
            'Hello ' + user.first_name + ',\n\n' +
            'Your ride to ' + ride.destination_city + ' on ' + ride.date_leaving.strftime(
        "%m/%d/%Y") + ' has been approved by a moderator. ' +
            'It will now be published on the HuskyHunt catalog.')
...
#   suspension_type: The type of suspension (seven, thirty, ban)
#   reason: A string giving the reason for suspension
def sendSuspendUserEmail(user, suspension_type, reason):
    # The body of the email
    message = ('Hello ' + user.first_name + ',\n\nYour account has been ')

    if suspension_type == "seven":
        message += 'suspended for seven days. '
    elif suspension_type == "thirty":
