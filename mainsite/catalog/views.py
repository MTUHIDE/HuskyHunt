...
def index(request):
    ...
    failed_search = None
    if request.session.has_key('index_redirect_failed_search') and request.session[
        'index_redirect_failed_search'] is not None:
...
def report_functionality(request, pk, item):
    ...
    # Check if user has maxed out on emails today
        ...
        messages.error(request, 'You can only report 5 posts per day! Please wait ' + strfdelta(time_remaining,
...
def email_functionality(request, pk, item, article, shortdesc, extra_tags):
    ...
    # Check if user has maxed out on emails today
        ...
        messages.error(request, 'You can only send ' + str(MAX_EMAIL_COUNT) + ' messages per day! Please wait ' + strfdelta(
            time_remaining, "{hours} hours and {minutes} minutes."), extra_tags=extra_tags)
